...
uint64_t static coo = 0;
uint64_t static ccc = 0;
...
public:
 ...
 //   hash_Utilization Utilization(void);
	...
	{
		hash_Utilization h;
       // std::cout<<"coo:"<<coo<<" ccc:" << ccc<<std::endl;
		//h.load_factor = (float)coo / map->capacity();
        h.load_factor = (float)coo / ccc *100; 
        //std::cout<<"ccc:"<< ccc << std::endl;
        h.utilization = ((float)(coo * 16) / (float)(ccc* (8+16) ) * 100);//8 Byte kVentryPointer and 16Byte KV——pair in PM for clevel hashing
        //h.utilization = ((float)(coo * 16) / (float)(map->capacity()* (8+16) ) * 100);//8 Byte kVentryPointer and 16Byte KV——pair in PM for clevel hashing */
		//return map->Utilization();
        return h;
	}
