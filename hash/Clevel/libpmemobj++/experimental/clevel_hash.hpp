...
foo();
                   ...
                   // std::cout << "size of level bucket: " << sizeof(bucket) << std::endl;
					// setup pool
                ...
                uint64_t
				totalBucketNum() const
				{
					return totalBucketNum(meta);
				}
                
                //function totalBucketNum was new added by renzhi xiao
                uint64_t
				totalBucketNum(level_meta_ptr_t m_copy) const
				{
					...
					for (li = m->last_level; li != m->first_level;)
					{
						...
						total_bucket_num += cl->capacity;
						li = cl->up;
					}
					total_bucket_num += li.get_address(my_pool_uuid)->capacity;
...
//the Utilization() function was added by rzxiao
/*                hash_Utilization Utilization(void){
                    hash_Utilization h;
                    h.load_factor = (float)valid_count / capacity(); 
                    h.utilization = ((float)(valid_count * 16) / (float)(capacity()* (8+16) ) * 100);//8 Byte kVentryPointer and 16Byte KV——pair in PM for clevel hashing
                    return h;
                }*/
				/**
