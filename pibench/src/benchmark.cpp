...
#pragma omp for schedule(static)
  ...
  std::cout << "\nOverview:"
   ...
   //for recovery         
  stopwatch_t sw_recovery;
  if(opt_.recovery){
    sw_recovery.start();
    hashtable_->recovery();
    auto recovery_time = sw_recovery.elapsed<std::chrono::nanoseconds>();
    std::cout <<"recovery time (ms):"<< recovery_time/1000000<< " milliseconds"<<std::endl;
  }
}
